...
void encrypt()
{
	...
	while (flg == 1)
	{
		for (size_t i = 0; i<strlen(ch1); i++)//把所输入的密钥转化为大写字母
		{
			if (ch1[i]>'z' || ch1[i]<'a')
			{
				cout << "请重新选择操作：" << endl;
		...
		if (flg == 1)
		{
			for (size_t i = 0; i<strlen(ch1); i++)//把密钥中的J都变为I
			{
				if (ch1[i] == 'J')ch1[i] = 'I';
			}
			int i = 0; int j = 0;
			//把密钥中的字母填入到矩阵中，并把该字母标记为已用
			for (size_t k = 0; k<strlen(ch1); k++)
			{
				for (int t = 0; t<25; t++)
				{
			...
			if (ch4 >= 'a')
				ch4 = ch4 - len;
			for (size_t k = 0; k<strlen(ch2); k++)//把所输入的明文转化为大写字母
			{
				if (ch2[k] >= 'a')
					ch2[k] = ch2[k] - len;
			}
			for (size_t k = 0; k<strlen(ch2); k++)//把明文中的J都变为I
			{
				if (ch2[k] == 'J')
					ch2[k] = 'I';
			}
			//为明文添加必要的无关字符以防止同一组的两个字符相同
			for (size_t k = 0; k<strlen(ch2); k += 2)
			{
				if (ch2[k] == ch2[k + 1])
				{
					for (size_t t = strlen(ch2); t>k; t--)
						ch2[t + 1] = ch2[t];
					ch2[k + 1] = ch4;
				}
			}
	...
	while (flg == 1)
	{
		...
		if (flg == 1)
		{
			...
			do{
				cout << "请输入密文（英文字符）：";
				cin >> ch2;
				for (size_t k = 0; k<strlen(ch2); k++)//把所输入的密文转化为大写字母
				{
					if (ch2[k] >= 'a')
						ch2[k] = ch2[k] - len;
				}
				for (size_t k = 0; k<strlen(ch2); k++)//把密文中的J都变为I
				{
					if (ch2[k] == 'J')ch2[k] = 'I';
				}
				for (size_t k = 0; k<strlen(ch2); k += 2)
